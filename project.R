...
# project: One thing that people regularly do is quantify how much of a particular activity they do, 
...
#Step 2
#Preprocessing steps which includes the elimination of NA values, all the variables that have near zero 
...
#Initialize the classifier based on a Random Forest model using a three cross validation process over train1 subset

Parameters <- trainControl(method="cv", number=3, verboseIter=F)
fit <- train(classe ~ ., data=train1, method="rf", trControl=Parameters )


#Print the obtained model to see the overall classifier accuracy
...
#From the model creation it can be observed thta the algorithm decid to create 500 trees, each one with 27 variables associated.

#Step 4

# Using the previously created model, the unpredicted instances are classified in order to obtain the labels for each of the 
...
#Step 6
#After  final quiz extract goldStandar to evaluate the accuracy 
